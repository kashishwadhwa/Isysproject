...
namespace WebApplication1
{
    public partial class dashboard : System.Web.UI.Page
    {
        ...
        public static string GetChart(string country)
        {
            ...
            {
                string query = string.Format("select CASE WHEN DATEPART(month,appointment_date)=1 THEN 'Jan' WHEN DATEPART(month,appointment_date)=2 THEN 'feb' WHEN DATEPART(month,appointment_date)=3 THEN 'Mar' WHEN DATEPART(month,appointment_date)=4 THEN 'Apr' WHEN DATEPART(month,appointment_date)=5 THEN 'May' WHEN DATEPART(month,appointment_date)=6 THEN 'Jun' WHEN DATEPART(month,appointment_date)=7 THEN 'Jul' WHEN DATEPART(month,appointment_date)=8 THEN 'Aug' WHEN DATEPART(month,appointment_date)=9 THEN 'Sep' WHEN DATEPART(month,appointment_date)=10 THEN 'Oct' WHEN DATEPART(month,appointment_date)=11 THEN 'Nov' WHEN DATEPART(month,appointment_date)=12 THEN 'Dec' END  as appointment_month ,count(appointment_id) as cnt_appointment  from appointment where DATEPART(year,appointment_date)={0} and DATEPART(month,appointment_date)<>9 group by DATEPART(month,appointment_date) order by DATEPART(month,appointment_date)", country);
